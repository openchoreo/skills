...
if [[ "$version" == "next" ]]; then
  ...
  if [[ -z "$version" ]]; then
    # default: newest stable minor. stable = plain vX.Y.x / vX.Y.Z with no
    # pre-release suffix. Match positively rather than denylisting suffixes
    # (alpha/beta/rc/m/…) — the project mints new ones a denylist misses.
