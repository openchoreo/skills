...
- **Trait override fields go under `parameters` (or `enabled` at the top level).** Use the trait's parameter schema (`get_trait_schema scope: cluster, name: <trait>`) to know what's overridable.
