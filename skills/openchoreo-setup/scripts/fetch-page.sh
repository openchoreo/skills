...
#
# Usage:
#   fetch-page.sh --title "<title>" [--version <minor>]
#   fetch-page.sh --list [--version <minor>]
#
# Exit codes:
...
if [[ -z "$version" ]]; then
  ...
  [[ $found -eq 1 ]] || die "version '$version' is not in versions.json (supported: ${available[*]})"
fi
...
# Match against the bracketed title in `- [Title](URL)` lines. Substring
# match is case-sensitive; the agent passes the exact title verbatim.
...
# `/docs/<version>/<path>`. Special-case the docs root (`/docs.md`)
# which would otherwise transform to `/docs/<version>.md`.
